...
/// 窓口日報出力のDAクラス
/// </summary>
public partial class S05_0509_DA : DataAccessorBase
{
    ...
    /// <returns></returns>
    public DataTable getSearchData(Hashtable param)
    {
        ...
        sqlString.AppendLine("   AND T_SEISAN_INFO.EIGYOSYO_CD = " + setParam("EIGYOSYO_CD", param["EigyosyoCd"], OracleDbType.Char, 2));
        sqlString.AppendLine("   AND T_SEISAN_INFO.CREATE_DAY >= " + setParam("CREATE_DAY_FROM", param["OutTaisyoMonDayFrom"], OracleDbType.Int32, 8));
        sqlString.AppendLine("   AND T_SEISAN_INFO.CREATE_DAY <= " + setParam("CREATE_DAY_TO", param["OutTaisyoMonDayTo"], OracleDbType.Int32, 8));
        sqlString.AppendLine("   AND T_SEISAN_INFO.DELETE_DAY = 0  ");
        sqlString.AppendLine("   AND NVL(T_SEISAN_INFO.URIAGE_KBN, ' ') <> 'V'  ");
        if (!string.IsNullOrWhiteSpace(Conversions.ToString(param["UserID"])))
        {
            sqlString.AppendLine("   AND T_SEISAN_INFO.ENTRY_PERSON_CD = " + setParam("ENTRY_PERSON_CD", param["UserID"], OracleDbType.Varchar2, 20));
        }
        ...
        try
        {
            result = base.getDataTable(sqlString.ToString());
        }
        catch (Exception ex)
        {
    ...
    /// <param name="param">営業所コード,出力対象月日(From),ユーザID,邦人/外客区分</param>
    /// <returns></returns>
    public DataTable getEigyosyoName(Hashtable param)
    {
        var sqlString = new StringBuilder();
        ...
        sqlString.AppendLine("    AND EM.EIGYOSYO_CD = " + setParam("EIGYOSYO_CD", param["EigyosyoCd"], OracleDbType.Char, 2));
        var result = new DataTable();
        try
        {
            result = base.getDataTable(sqlString.ToString());
    ...
    /// <returns></returns>
    public DataTable getPrintOffUtiwake(Hashtable param)
    {
        var sqlString = new StringBuilder();
        // 借方
        ...
        sqlString.AppendLine("   AND SI.EIGYOSYO_CD = " + setParam("EIGYOSYO_CD", param["EigyosyoCd"], OracleDbType.Char, 2));
        sqlString.AppendLine("   AND SI.CREATE_DAY >= " + setParam("CREATE_DAY_FROM", param["OutTaisyoMonDayFrom"], OracleDbType.Int32, 8));
        sqlString.AppendLine("   AND SI.CREATE_DAY <= " + setParam("CREATE_DAY_TO", param["OutTaisyoMonDayTo"], OracleDbType.Int32, 8));
        sqlString.AppendLine("   AND SI.DELETE_DAY = 0  ");
        sqlString.AppendLine("   AND NVL(SI.URIAGE_KBN, ' ') <> 'V'  ");
        if (!string.IsNullOrWhiteSpace(Conversions.ToString(param["CrsKbn"])))
        {
            sqlString.AppendLine("   AND (T_CRS_LEDGER_BASIC.HOUJIN_GAIKYAKU_KBN IS NULL ");
            sqlString.AppendLine("    OR  T_CRS_LEDGER_BASIC.HOUJIN_GAIKYAKU_KBN = " + setParam("HOUJIN_GAIKYAKU_KBN", param["CrsKbn"], OracleDbType.Char, 1) + ") ");
        }

        if (!string.IsNullOrWhiteSpace(Conversions.ToString(param["UserID"])))
        {
            sqlString.AppendLine("   AND SI.ENTRY_PERSON_CD = " + setParam("ENTRY_PERSON_CD", param["UserID"], OracleDbType.Varchar2, 20));
        }

        List<string> checkoutItemSignonTime = (List<string>)param["SignonTime"];
        if (checkoutItemSignonTime.Count > 0)
        {
            sqlString.AppendLine("   AND SI.SIGNON_TIME IN (");
            for (int i = 0, loopTo = checkoutItemSignonTime.Count - 1; i <= loopTo; i++)
            {
                if (i > 0)
                {
        ...
        // sql実行
        DataTable result = base.getDataTable(sqlString.ToString());
        // リターン
        return result;
    }
    ...
    /// <param name="param">営業所コード,出力対象月日(From),出力対象月日(To),ユーザID,邦人/外客区分,サインオン時間</param>
    /// <returns></returns>
    public DataTable getPrintOffSeisanInfo(Hashtable param)
    {
        var sqlString = new StringBuilder();
        sqlString.AppendLine(" SELECT  ");
    ...
    /// <param name="param">営業所コード,出力対象月日(From),ユーザID,邦人/外客区分</param>
    /// <returns></returns>
    public DataTable getPrintONUtiwake(Hashtable param)
    {
        var sqlString = new StringBuilder();
        // 借方
        sqlString.AppendLine(" SELECT   ");
    ...
    /// <param name="param">営業所コード,出力対象月日(From),ユーザID,邦人/外客区分</param>
    /// <returns></returns>
    public DataTable getPrintONSeisanInfo(Hashtable param)
    {
        var sqlString = new StringBuilder();
        // 借方
        sqlString.AppendLine(" SELECT   ");
    ...
    /// <returns></returns>
    public DataTable getSonotaPrintAriNasi(Hashtable param)
    {
        var sqlString = new StringBuilder();
        sqlString.AppendLine(" SELECT ");
