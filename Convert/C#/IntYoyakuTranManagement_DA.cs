...
/// </remarks>
/// </summary>
public partial class IntYoyakuTranManagement_DA : DataAccessorBase
{

    #region  定数／変数 
    private TIntYoyakuTranManagementEntity clsIntYoyakuTranManagementEntity = new TIntYoyakuTranManagementEntity();
    ...
    /// <returns></returns>
    public DataTable accessIntYoyakuTranManagement(accessType type, Hashtable paramInfoList = null)
    {
        // SQL文字列
    ...
    /// <param name="paramInfoList"></param>
    /// <remarks></remarks>
    private string getChargeApplicationNinzu(Hashtable paramInfoList)
    {
        var sqlString = new StringBuilder();
        {
            var withBlock = clsIntYoyakuTranManagementEntity;
            // SELECT
            ...
            // WHERE 
            if (paramInfoList is object && paramInfoList.Count > 0)
            {
                sqlString.AppendLine(" WHERE ");
                if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["tranKey"])))
                {
                    sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.TRAN_KEY =  CAST( " + setParam("tranKey", paramInfoList["tranKey"], withBlock.tranKey.DBType, withBlock.tranKey.IntegerBu, withBlock.tranKey.DecimalBu) + " AS Char(46))");
                }

                if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["tranKbn"])))
                {
                    sqlString.AppendLine("    AND T_INT_YOYAKU_TRAN_MANAGEMENT.TRAN_KBN =  CAST( " + setParam("tranKbn", paramInfoList["tranKbn"], withBlock.tranKbn.DBType, withBlock.tranKbn.IntegerBu, withBlock.tranKbn.DecimalBu) + " AS Char(1))");
                }
            }
    ...
    /// <param name="paramInfoList"></param>
    /// <remarks></remarks>
    private string getIntYoyakuTranLogDetail(Hashtable paramInfoList)
    {
        var sqlString = new StringBuilder();
        {
            var withBlock = clsIntYoyakuTranManagementEntity;
            ...
            if (paramInfoList is object && paramInfoList.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["tranKey"])))
                {
                    sqlString.AppendLine("    AND T_INT_YOYAKU_TRAN_MANAGEMENT.TRAN_KEY =  CAST( " + setParam("tranKey", paramInfoList["tranKey"], withBlock.tranKey.DBType, withBlock.tranKey.IntegerBu, withBlock.tranKey.DecimalBu) + " AS Char(46))");
                }
    ...
    /// <param name="paramInfoList"></param>
    /// <remarks></remarks>
    private string getIntYoyakuTranLog(Hashtable paramInfoList)
    {
        var sqlString = new StringBuilder();
        {
            var withBlock = clsIntYoyakuTranManagementEntity;
            ...
            sqlString.AppendLine("    And T_INT_YOYAKU_TRAN_MANAGEMENT.YOYAKU_NO = T_YOYAKU_INFO_BASIC.YOYAKU_NO) ");
            sqlString.AppendLine("WHERE 1 = 1");
            if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["createDay"])))
            {
                // インターネット予約トランザクション管理．作成日 = 画面．処理受付日
                sqlString.AppendLine("    AND T_INT_YOYAKU_TRAN_MANAGEMENT.CREATE_DAY = " + setParam("createDay", paramInfoList["createDay"], withBlock.CreateDay.DBType, withBlock.CreateDay.IntegerBu, withBlock.CreateDay.DecimalBu));
            }

            if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["crsSyuptDay"])))
            {
                // インターネット予約トランザクション管理．出発日 = 画面．出発日
                sqlString.AppendLine("    AND T_INT_YOYAKU_TRAN_MANAGEMENT.CRS_SYUPT_DAY = " + setParam("crsSyuptDay", paramInfoList["crsSyuptDay"], withBlock.CrsSyuptDay.DBType, withBlock.CrsSyuptDay.IntegerBu, withBlock.CrsSyuptDay.DecimalBu));
            }

            if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["crsCd"])))
            {
                // インターネット予約トランザクション管理．コースコード = 画面．コースコード
                sqlString.AppendLine("    AND T_INT_YOYAKU_TRAN_MANAGEMENT.CRS_CD = " + setParam("crsCd", Conversions.ToString(paramInfoList["crsCd"]), withBlock.CrsCd.DBType, withBlock.CrsCd.IntegerBu, withBlock.CrsCd.DecimalBu));
            }

            if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["yoyakuNo"])))
            {
                // インターネット予約トランザクション管理．予約ＮＯ = 画面．「予約番号」の1桁目以外
                sqlString.AppendLine("    AND T_INT_YOYAKU_TRAN_MANAGEMENT.YOYAKU_NO = " + setParam("yoyakuNo", Conversions.ToString(paramInfoList["yoyakuNo"]), withBlock.YoyakuNo.DBType, withBlock.YoyakuNo.IntegerBu, withBlock.YoyakuNo.DecimalBu));
            }

            if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["yoyakuKbn"])))
            {
                // インターネット予約トランザクション管理．予約区分 = 画面．「予約番号」の1桁目
                sqlString.AppendLine("    AND T_INT_YOYAKU_TRAN_MANAGEMENT.YOYAKU_KBN = " + setParam("yoyakuKbn", Conversions.ToString(paramInfoList["yoyakuKbn"]), withBlock.YoyakuKbn.DBType, withBlock.YoyakuKbn.IntegerBu, withBlock.YoyakuKbn.DecimalBu));
            }

            if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["yykmksName"])))
            {
                // インターネット予約トランザクション管理.申込者名 Like 画面.名前%
                sqlString.AppendLine("    AND T_INT_YOYAKU_TRAN_MANAGEMENT.YYKMKS_NAME LIKE " + setParam("yykmksName", paramInfoList["yykmksName"], withBlock.YykmksName.DBType) + "||'%'");
            }

            if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["agentTelNo"])))
            {
                // 予約情報（基本）．業者電話番号 LIKE 画面．電話番号%
                sqlString.AppendLine("    AND T_YOYAKU_INFO_BASIC.AGENT_TEL_NO LIKE " + setParam("agentTelNo", paramInfoList["agentTelNo"], clsTYoyakuInfoBasicEntity.AgentTelNo.DBType) + "||'%'");
            }

            if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["agentCd"])))
            {
                // 予約情報（基本）．業者コード = 画面．業者コード
                sqlString.AppendLine("    AND T_YOYAKU_INFO_BASIC.AGENT_CD = " + setParam("agentCd", Conversions.ToString(paramInfoList["agentCd"]), withBlock.AgentCd.DBType, withBlock.AgentCd.IntegerBu, withBlock.AgentCd.DecimalBu));
            }

            if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["chkTaiouAlreadyWith"])))
            {
            ...
            else if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["chkTaiouHuyoWith"])))
            {
    ...
    /// <param name="paramInfoList"></param>
    /// <remarks></remarks>
    private string getIntYoyakuTranManagement(Hashtable paramInfoList)
    {
        var sqlString = new StringBuilder();
        {
            var withBlock = clsIntYoyakuTranManagementEntity;
            ...
            if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["createDayFrom"])))
            {
                sqlString.AppendLine("    AND T_INT_YOYAKU_TRAN_MANAGEMENT.CREATE_DAY >= " + setParam("createDayFrom", paramInfoList["createDayFrom"], withBlock.createDay.DBType, withBlock.createDay.IntegerBu, withBlock.createDay.DecimalBu));
            }

            if (!string.IsNullOrEmpty(Conversions.ToString(paramInfoList["createDayTo"])))
            {
                sqlString.AppendLine("    AND T_INT_YOYAKU_TRAN_MANAGEMENT.CREATE_DAY <= " + setParam("createDayTo", paramInfoList["createDayTo"], withBlock.createDay.DBType, withBlock.createDay.IntegerBu, withBlock.createDay.DecimalBu));
            }
            // ORDER
    ...
    /// <returns></returns>
    /// <remarks></remarks>
    public int executeIntYoyakuTranManagement(accessType type, Hashtable paramInfoList = null)
    {
        int returnValue = 0;
        string sqlString = string.Empty;
        try
    ...
    /// <returns></returns>
    /// <remarks></remarks>
    private string executeUpdateIntYoyokuTranLogDetail(Hashtable paramInfoList)
    {
        var sqlString = new StringBuilder();
        {
            var withBlock = clsIntYoyakuTranManagementEntity;
            ...
            sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.OPERATION_TAISHO_FLG	= " + setParam("operationTaishoFlg", paramInfoList["operationTaishoFlg"], withBlock.operationTaishoFlg.DBType, withBlock.operationTaishoFlg.IntegerBu, withBlock.operationTaishoFlg.DecimalBu) + ",");
            sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.INVALID_FLG	= " + setParam("invalidFlg", paramInfoList["invalidFlg"], withBlock.invalidFlg.DBType, withBlock.invalidFlg.IntegerBu, withBlock.invalidFlg.DecimalBu) + ",");
            sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.OPERATION_TAISHO_DAY	= " + setParam("operationTaishoDay", paramInfoList["operationTaishoDay"], withBlock.operationTaishoDay.DBType, withBlock.operationTaishoDay.IntegerBu, withBlock.operationTaishoDay.DecimalBu) + ",");
            sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.OPERATION_TAISHO_TIME = " + setParam("operationTaishoTime", paramInfoList["operationTaishoTime"], withBlock.operationTaishoTime.DBType, withBlock.operationTaishoTime.IntegerBu, withBlock.operationTaishoTime.DecimalBu) + ",");
            sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.OPERATION_TAISHO_PERSON = " + setParam("operationTaishoPerson", paramInfoList["operationTaishoPerson"], withBlock.operationTaishoPerson.DBType, withBlock.operationTaishoPerson.IntegerBu, withBlock.operationTaishoPerson.DecimalBu) + ",");
            sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.OPERATION_TAISHO_PGMID = " + setParam("operationTaishoPgmid", paramInfoList["operationTaishoPgmid"], withBlock.operationTaishoPgmid.DBType, withBlock.operationTaishoPgmid.IntegerBu, withBlock.operationTaishoPgmid.DecimalBu) + ",");
            sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.UPDATE_DAY = " + setParam("updateDay", paramInfoList["updateDay"], withBlock.updateDay.DBType, withBlock.updateDay.IntegerBu, withBlock.updateDay.DecimalBu) + ",");
            sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.UPDATE_TIME = " + setParam("updateTime", paramInfoList["updateTime"], withBlock.updateTime.DBType, withBlock.updateTime.IntegerBu, withBlock.updateTime.DecimalBu) + ",");
            sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.UPDATE_PERSON = " + setParam("updatePerson", paramInfoList["updatePerson"], withBlock.updatePerson.DBType, withBlock.updatePerson.IntegerBu, withBlock.updatePerson.DecimalBu) + ",");
            sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.UPDATE_PGMID = " + setParam("updatePgmid", paramInfoList["updatePgmid"], withBlock.updatePgmid.DBType, withBlock.updatePgmid.IntegerBu, withBlock.updatePgmid.DecimalBu));
            // WHERE 
            sqlString.AppendLine(" WHERE ");
            sqlString.AppendLine("    T_INT_YOYAKU_TRAN_MANAGEMENT.TRAN_KEY =  CAST( " + setParam("tranKey", paramInfoList["tranKey"], withBlock.tranKey.DBType, withBlock.tranKey.IntegerBu, withBlock.tranKey.DecimalBu) + " AS Char(46))");
            sqlString.AppendLine("    AND T_INT_YOYAKU_TRAN_MANAGEMENT.TRAN_KBN =  CAST( " + setParam("tranKbn", paramInfoList["tranKbn"], withBlock.tranKbn.DBType, withBlock.tranKbn.IntegerBu, withBlock.tranKbn.DecimalBu) + " AS Char(1))");
